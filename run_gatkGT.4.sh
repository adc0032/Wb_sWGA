...
srun java -jar /cm/shared/apps/GenomeAnalysisTk/3.5/GenomeAnalysisTK.jar -nt 20 -T GenotypeGVCFs -R Wb-PNG_Genome_assembly-pt22.spades.ragoutrep.gapfill.mt.fasta --variant $( ls gatk_gvcf/*.${SLURM_ARRAY_TASK_ID}.HC-all.gvcf | cut -d "." -f1 | cut -d "/" -f2 ).${SLURM_ARRAY_TASK_ID}.HC-all.gvcf -o $( ls gatk_gvcf/*.${SLURM_ARRAY_TASK_ID}.HC-all.gvcf | cut -d "." -f1 | cut -d "/" -f2 ).HC-all.vcf
