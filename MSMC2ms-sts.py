...
parser.add_argument("--npops", help="Length of chromosome to simulate",type=int,default=1)
args = parser.parse_args()


'''note on cross coalescent: There is no exact split time, and CC only marks the decay of relatedness between 2 pops
and mutation segregate and lineages find MRCA within pops rather than among. The mean CC time may indeed be a 'split'
if we refer to this as a barrier or something. We know that reconstructed, when CC == 1 the population is together, 
so -ej t 1 2; Likewise at time CC == 0 there is no more migration between the 2 populations. -eM t 0; also will need -en t 1 size.
so basically: -I 2 theta00 theta11/theta00 0 -en t 1 size -en t 2 size -eM t 4Nm -ej t 1 2
### might not be able to estimate 4Nm, or specific times but can estimate the decay rate, which will serve as a prior in ABC ...'''
