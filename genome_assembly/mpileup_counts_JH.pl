...
print join("\t","chromosome","position","coverage","deleted","reference","raf",@bases)."\n";
#print join("\t","chromosome","position","coverage","reference","raf",@bases,"deleted")."\n";
my %sequences;
while(<>){
  #parse mpileup
  ...
  #print data
    print join("\t", $chr, $pos, $coverage, $deleted, $ref,sprintf("%2f", $counts{$ref}/$total), map { $counts{$_} } @bases), "\n";
    # print join("\t", $chr, $pos, $coverage, $ref,sprintf("%2f", $counts{$ref}/$total), map { $counts{$_} } @bases, $deleted), "\n";
}
