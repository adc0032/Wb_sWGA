...
def vcf2fasta(fastaFile, vcfdict, phased, NaRef):
...
def vcfsample(vcf, fastaFile, NaRef, iupac, major, randallele, phased):
    ...
    vcf2fasta(fastaFile, vcfdict, phased, NaRef)
    return(None)
...
if __name__ == '__main__':
    if args.ancestral:
        vcfAAsample(args.vcfFile, args.fasta)
    else:
        if args.phased:
            assert args.phased > (args.iupac + args.major + args.rand)
        vcfsample(args.vcfFile, args.fasta, args.NaRef, args.iupac,
                  args.major, args.rand, args.phased)
